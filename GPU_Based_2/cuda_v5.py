...
def makeVf(Ni, q, m, l, wr, offsetr, offsetz, vbumpr, vbumpz):
    ...
    for i in range(Ni):
        ...
        # Assign distinct initial z-velocities to each ion to facilitate reordering
        vf[i, 3] = 1e-2 * (i + 1)  # Example: 0.01, 0.02, 0.03 m/s
...
def solveFields_cuda(vf, ErDC, EzDC, ErAC, EzAC, Nrmid, Nzmid, Ni, dr, dz, Erf2, Ezf2):
    ...
    Nz = ErDC.shape[1]
    for i in range(Ni):
        jCell = ptovPos_cuda(vf[i, 0], Nrmid, dr)
        kCell = ptovPos_cuda(vf[i, 1], Nzmid, dz)
        # Bounds checking
...
def solveFields_cuda(vf, ErDC, EzDC, ErAC, EzAC, Nrmid, Nzmid, Ni, dr, dz, Erf2, Ezf2):
    ...
    for i in range(Ni):
        ...
        jCell_f = ptovPos_cuda(vf[i, 0], Nrmid, dr)
        kCell_f = ptovPos_cuda(vf[i, 1], Nzmid, dz)
        ...
        # Assign electric fields without scaling
        Erf2[i] = ErDC[jCell, kCell] + ErAC[jCell, kCell]
        Ezf2[i] = EzDC[jCell, kCell] + EzAC[jCell, kCell]

        for j in range(Ni):
            if j != i and vf[j, 5] < 1e6:
                rdist = vf[j, 0] - vf[i, 0]
                zdist = vf[j, 1] - vf[i, 1]
                sqDist = rdist * rdist + zdist * zdist
                dist = math.sqrt(sqDist)
...
def collisionParticlesFields_cuda(vf, vc, Ni, ErDC, EzDC, ErAC, EzAC, dr, dz, dtNow, Nrmid, Nzmid, Nr, Nz, Erfi, Ezfi):
    ...
    for i in range(Nc):
        # Convert positions to integer grid indices
        jCell_f = ptovPos_cuda(vc[i, 0], Nrmid, dr)
        kCell_f = ptovPos_cuda(vc[i, 1], Nzmid, dz)
        jCell = int(jCell_f + 0.5)
        kCell = int(kCell_f + 0.5)
        ...
        Erfc1 = ((ErDC[jCell + 1, kCell] + ErAC[jCell + 1, kCell]) - 
                 (ErDC[jCell - 1, kCell] + ErAC[jCell - 1, kCell])) / dr
        Ezfc1 = ((EzDC[jCell, kCell + 1] + EzAC[jCell, kCell + 1]) - 
                 (EzDC[jCell, kCell - 1] + EzAC[jCell, kCell - 1])) / dz
        ...
        for j in range(Ni):
            ...
            dist = math.sqrt(sqDist)
            if dist < 1e-12:
                continue  # Avoid division by zero or infinite forces
            Rhatr = rdist / dist
            Rhatz = zdist / dist
            Erfi[j] += -pR * (2 * Rhatr) / (C1 * dist ** 3)
            Ezfi[j] += -pZ * (2 * Rhatz) / (C1 * dist ** 3)
...
def mcCollision_kernel(vf_all, rc_all, zc_all, vrc_all, vzc_all, qc, mc, ac, Nt_all, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder_all):
    ...
    if idx < vf_all.shape[0]:
        # Access the ion parameters for this shot
        vf = vf_all[idx, :]
        rc = rc_all[idx]
        zc = zc_all[idx]
        vrc = vrc_all[idx]
...
def mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields):
    ...
    dtNow = dtSmall
    # Initialize local arrays for fields
    Erfi = cuda.local.array(3, dtype=float64)  # Assuming Ni=3
    Ezfi = cuda.local.array(3, dtype=float64)
    ...
    for step in range(Nt):
        ...
        if vc[0, 5] < 1e6:
            # Handle collisional particles
            collisionParticlesFields_cuda(vf, vc, Ni, nullFields, DC, RF, nullFields, dr, dz, dtNow, Nrmid, Nzmid, Nr, Nz, Erfi, Ezfi)
            updatePoss_cuda(vc, dr, dz, dtNow, Nr, Nz, Nrmid, Nzmid)
        else:
            dtNow = dtLarge
        ...
        # Check for reordering
        for j in range(1, Ni):
            if vf[j, 5] >= 1e6 or vf[j - 1, 5] >= 1e6:
                continue  # Skip ejected ions
            if vf[j, 1] > vf[j - 1, 1]:
                reorder += 1
                Nt = step + 1000  # Extending simulation steps (though loop breaks)
                break
