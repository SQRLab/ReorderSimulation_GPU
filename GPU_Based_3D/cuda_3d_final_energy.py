...
@cuda.jit(device=True)
def calculate_kinetic_energy_cuda(vf):
    ...
    for i in range(Ni):
        # KE = Σ(1/2 * m * v²)
        v_squared = vf[i,3]*vf[i,3] + vf[i,4]*vf[i,4] + vf[i,5]*vf[i,5]
        KE += 0.5 * vf[i,7] * v_squared
        
    return KE

@cuda.jit(device=True)
def calculate_potential_energy_cuda(vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz):
    """Calculate total potential energy of the system including RF trap, DC trap, and Coulomb interactions.
    
    Parameters:
        vf: Array containing ion information [x,y,z,vx,vy,vz,q,m,alpha]
        RFx, RFy: RF field arrays
        DC: DC field array
        Nrmid, Nzmid: Grid midpoints
        dr, dz: Grid spacings
    ...
    for i in range(Ni):
        # Get grid cell indices for current ion position
        jCell = int(round(ptovPos_cuda(vf[i,0], Nrmid, dr)))
        iCell = int(round(ptovPos_cuda(vf[i,1], Nrmid, dr)))
        kCell = int(round(ptovPos_cuda(vf[i,2], Nzmid, dz)))
        
        # Get field values at ion position
        rf_x = RFx[jCell,iCell,kCell]
        rf_y = RFy[jCell,iCell,kCell]
        dc_z = DC[jCell,iCell,kCell]
        
        # RF pseudopotential energy (quadratic in field)
        # PE_rf = q²|E|²/(4mω²) for each RF component
        # Factor of 1/2 because E = E₀cos(ωt) and we average over RF cycle
        omega_rf = 2.0 * π * 5e6  # RF drive frequency
        PE += 0.25 * vf[i,6] * vf[i,6] * (rf_x * rf_x + rf_y * rf_y) / (vf[i,7] * omega_rf * omega_rf)
        
        # DC potential energy
        # For harmonic trap, PE = qE₀z
        PE += vf[i,6] * dc_z * vf[i,2]
        
        # Coulomb potential energy between ions
        # Only count each interaction once (j > i)
        for j in range(i + 1, Ni):
            dx = vf[i,0] - vf[j,0]
            dy = vf[i,1] - vf[j,1]
            dz = vf[i,2] - vf[j,2]
            ...
            if r > 0:  # Avoid division by zero
                # Coulomb potential = q₁q₂/4πε₀r
                PE += vf[i,6] * vf[j,6] / (4.0 * π * eps0 * r)
    
    return PE

@cuda.jit(device=True)
def calculate_collision_particle_kinetic_energy_cuda(vc):
    """Calculate kinetic energy of collision particle using mass and velocity"""
    # KE = 1/2 * m * (vx² + vy² + vz²)
    v_squared = vc[0,3]*vc[0,3] + vc[0,4]*vc[0,4] + vc[0,5]*vc[0,5]
    KE = 0.5 * vc[0,7] * v_squared
    return KE

@cuda.jit(device=True)
def calculate_collision_particle_potential_energy_cuda(vc, vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz):
    """Calculate potential energy of collision particle including all interactions"""
    PE = 0.0
    eps0 = 8.854e-12
    Ni = current_ion
    
    # Get grid cell indices for collision particle position
    jCell = int(round(ptovPos_cuda(vc[0,0], Nrmid, dr)))
    iCell = int(round(ptovPos_cuda(vc[0,1], Nrmid, dr)))
    kCell = int(round(ptovPos_cuda(vc[0,2], Nzmid, dz)))
    
    # 1. Trap potential if particle is charged
    if vc[0,6] != 0.0:
        rf_x = RFx[jCell,iCell,kCell]
        rf_y = RFy[jCell,iCell,kCell]
        dc_z = DC[jCell,iCell,kCell]
        PE += vc[0,6] * (rf_x * vc[0,0] + rf_y * vc[0,1] + dc_z * vc[0,2])
    
    # 2. Ion-neutral interaction through induced dipole
    alpha = vc[0,8]  # Polarizability
    for j in range(Ni):
        dx = vc[0,0] - vf[j,0]
        dy = vc[0,1] - vf[j,1]
        dz = vc[0,2] - vf[j,2]
        ...
        # Calculate ion's electric field at collision particle position
        E_ion_squared = (vf[j,6]/(4 * π * eps0 * r * r))**2
        
        # Induced dipole potential = -1/2 * α * E²
        PE += -0.5 * alpha * E_ion_squared
    
    return PE

@cuda.jit(device=True)
def calculate_total_system_energy_cuda(vf, vc, RFx, RFy, DC, Nrmid, Nzmid, dr, dz):
    """Calculate total energy of the entire system"""
    # Ion energies
    ion_KE = calculate_kinetic_energy_cuda(vf)
    ion_PE = calculate_potential_energy_cuda(vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz)
    
    # Collision particle energies
    coll_KE = calculate_collision_particle_kinetic_energy_cuda(vc)
    coll_PE = calculate_collision_particle_potential_energy_cuda(vc, vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz)
    
    return ion_KE + ion_PE + coll_KE + coll_PE
...
def mcCollision_cuda(vf, xc, yc, zc, vxc, vyc, vzc, qc, mc, ac, Nt, dtSmall, 
                    ...
                    storage_params, termination_reason):
    """Modified collision simulation function with complete energy tracking"""
    reorder[0] = 0
    ...
    base_interval = storage_params[0]
    collision_interval = storage_params[1]
    total_storage_steps = storage_params[2]
    ...
    for i in range(Ni):
        positions_over_time[storage_step, i, 0] = vf[i, 0]
        positions_over_time[storage_step, i, 1] = vf[i, 1]
        positions_over_time[storage_step, i, 2] = vf[i, 2]
    
    positions_over_time[storage_step, 2, 0] = vc[0, 0]
    positions_over_time[storage_step, 2, 1] = vc[0, 1]
    positions_over_time[storage_step, 2, 2] = vc[0, 2]

    # Store initial energies
    energies_over_time[storage_step, 0] = calculate_kinetic_energy_cuda(vf)                     # Ion KE
    energies_over_time[storage_step, 1] = calculate_potential_energy_cuda(vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz)   # Ion PE
    energies_over_time[storage_step, 2] = energies_over_time[storage_step, 0] + energies_over_time[storage_step, 1]  # Ion Total
    
    energies_over_time[storage_step, 3] = calculate_collision_particle_kinetic_energy_cuda(vc)   # Collision KE
    energies_over_time[storage_step, 4] = calculate_collision_particle_potential_energy_cuda(vc, vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz)  # Collision PE
    energies_over_time[storage_step, 5] = energies_over_time[storage_step, 3] + energies_over_time[storage_step, 4]  # Collision Total
    ...
    while accumulated_time < total_physical_time:
        # Check storage space
        if storage_step >= total_storage_steps - 1:
            termination_reason[0] = 1  # Storage limit reached
            break
        ...
        # Adaptive storage based on collision mode
        store_this_step = False
        if not collision_mode:
            compression_counter += 1
            if compression_counter >= base_interval:
                store_this_step = True
                compression_counter = 0
        else:
            compression_counter += 1
            if compression_counter >= collision_interval:
                store_this_step = True
                compression_counter = 0
        ...
        if store_this_step:
            ...
            energies_over_time[storage_step, 0] = calculate_kinetic_energy_cuda(vf)
            energies_over_time[storage_step, 1] = calculate_potential_energy_cuda(vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz)
            energies_over_time[storage_step, 2] = energies_over_time[storage_step, 0] + energies_over_time[storage_step, 1]
            
            energies_over_time[storage_step, 3] = calculate_collision_particle_kinetic_energy_cuda(vc)
            energies_over_time[storage_step, 4] = calculate_collision_particle_potential_energy_cuda(vc, vf, RFx, RFy, DC, Nrmid, Nzmid, dr, dz)
            energies_over_time[storage_step, 5] = energies_over_time[storage_step, 3] + energies_over_time[storage_step, 4]
        ...
        if mass_sum > 1e5:  # Ion ejection
            reorder[0] += 2
            termination_reason[0] = 2
            break

        for i in range(1, Ni):
            if vf[i, 2] > vf[i-1, 2] and crossTest < 1:
                reorder[0] += 1
                termination_reason[0] = 3
                crossTest += 1
                break
    ...
    # Store final step count and mark completion
...
def mcCollision_kernel(vf_all, xc_all, yc_all, zc_all, vxc_all, vyc_all, vzc_all, 
                      ...
                      total_physical_time, positions_over_time, timesteps, energies_over_time,
                      storage_params, termination_reason_all):
    """Kernel function for running collision simulations"""
    ...
    if thread_id < vf_all.shape[0]:
        ...
        termination_reason = cuda.local.array(1, dtype=int32)
        ...
        mcCollision_cuda(
            ...
            reorder, withCollision, progress, total_physical_time,
            thread_positions, thread_timesteps, thread_energies,
            storage_params, termination_reason
        )
        
        # Copy results back
        reorder_all[thread_id] = reorder[0]
        termination_reason_all[thread_id] = termination_reason[0]
        for i in range(current_ion):
            for j in range(9):
