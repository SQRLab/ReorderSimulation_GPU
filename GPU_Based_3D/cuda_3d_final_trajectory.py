...
def mcCollision_cuda(vf, xc, yc, zc, vxc, vyc, vzc, qc, mc, ac, Nt, dtSmall, 
                   ...
                   positions_over_time, timesteps, storage_params, termination_reason):
    '''
    ...
    max_storage_steps = storage_params[2]

    # Store initial state
    if storage_step < max_storage_steps:
        timesteps[storage_step] = accumulated_time
        positions_over_time[storage_step, 0, 0] = vf[0, 0]
        positions_over_time[storage_step, 0, 1] = vf[0, 1]
        positions_over_time[storage_step, 0, 2] = vf[0, 2]
        positions_over_time[storage_step, 1, 0] = vf[1, 0]
        positions_over_time[storage_step, 1, 1] = vf[1, 1]
        positions_over_time[storage_step, 1, 2] = vf[1, 2]
    ...
    while accumulated_time < total_physical_time and timestep_counter < Nt:
        timestep_counter += 1
        ...
        if math.isnan(dtNow) or math.isinf(dtNow):
            termination_reason[0] = 5  # Numerical instability
            break
        ...
        # Adaptive storage based on simulation mode
        store_this_step = False
        if collision_mode:
            if timestep_counter % collision_interval == 0:
                store_this_step = True
        else:
            if timestep_counter % base_interval == 0:
                store_this_step = True

        if store_this_step and storage_step < max_storage_steps:
            timesteps[storage_step] = accumulated_time
            positions_over_time[storage_step, 0, 0] = vf[0, 0]
            positions_over_time[storage_step, 0, 1] = vf[0, 1]
            positions_over_time[storage_step, 0, 2] = vf[0, 2]
    ...
    if termination_reason[0] == 0:  # If not set by other conditions
        if timestep_counter >= Nt:
            termination_reason[0] = 4  # Reached max timesteps
        elif accumulated_time >= total_physical_time:
            termination_reason[0] = 0  # Normal completion

    # Store final step count and actual time reached
    timesteps[max_storage_steps-1] = float64(storage_step)  # Store actual number of steps used
    timesteps[max_storage_steps-2] = accumulated_time  # Store final physical time reached
...
def mcCollision_kernel(vf_all, xc_all, yc_all, zc_all, vxc_all, vyc_all, vzc_all, 
                     ...
                     storage_params, termination_reasons):
    '''
    CUDA kernel for Monte Carlo collision simulation with trajectory tracking
    
    New Parameters:
        storage_params: Array with [base_interval, collision_interval, max_storage_steps]
        termination_reasons: Array to store termination reason for each thread
    '''
    ...
    if thread_id < vf_all.shape[0]:
        ...
        termination_reason[0] = 0  # Initialize to 0
        ...
        mcCollision_cuda(
            ...
            reorder, withCollision, progress, total_physical_time,
            thread_positions, thread_timesteps, storage_params,
            termination_reason
        )
        
        # Copy results back to global memory
        reorder_all[thread_id] = reorder[0]
        termination_reasons[thread_id] = termination_reason[0]
        for i in range(current_ion):
            for j in range(9):
