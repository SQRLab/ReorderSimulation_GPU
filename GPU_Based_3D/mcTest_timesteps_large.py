...
import time
from cuda_3d_final_large import *
import random
import json
import os
import subprocess
...
def run_simulation(Nr, Nz, shots):
    """
...
def main():
    ...
    os.makedirs("timestep_results_new_large", exist_ok=True)
    
    # Create results file with current timestep values in filename
    results_file = f"timestep_results_new_large/results_dtSmall_{dtSmall}_dtLarge_{dtLarge}_dtCollision_{dtCollision}_Nt_{Nt}.json"
    ...
    for Nr in Nr_values:
        for Nz in Nz_values:
            ...
            reorders, ejections = run_simulation(Nr, Nz, shot_sizes[0])
