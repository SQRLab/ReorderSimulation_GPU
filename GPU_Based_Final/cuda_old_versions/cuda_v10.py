...
@cuda.jit(device=True)
def mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder, sum_masses, Erfi_history, Ezfi_history, pos_history, vel_history):
...
@cuda.jit
def mcCollision_kernel(vf_all, rc_all, zc_all, vrc_all, vzc_all, qc, mc, ac, Nt_all, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder_all, sum_masses_all, Erfi_history_all, Ezfi_history_all, pos_history_all, vel_history_all):
    idx = cuda.grid(1)
    if idx < vf_all.shape[0]:
        ...
        mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision,
                        nullFields, reorder, sum_masses, Erfi_history, Ezfi_history, pos_history, vel_history)
...
def main():
    ...
    # Allocate device arrays for histories
    ...
    pos_history_all = cuda.device_array((shots, Nt * Ni, 2), dtype=np.float64)  # Positions: r and z
    vel_history_all = cuda.device_array((shots, Nt * Ni, 2), dtype=np.float64)  # Velocities: vr and vz

    threads_per_block = 1
    blocks = shots  # Number of simulations

    mcCollision_kernel[blocks, threads_per_block](
        vf_device, rc_all, zc_all, vrc_all, vzc_all,
        ...
        Erfi_history_all, Ezfi_history_all, pos_history_all, vel_history_all
    )

    # Copy the results back to the host
    Erfi_history = Erfi_history_all.copy_to_host()
    Ezfi_history = Ezfi_history_all.copy_to_host()
    pos_history = pos_history_all.copy_to_host()
    vel_history = vel_history_all.copy_to_host()

    # Print Erfi, Ezfi, positions, and velocities for each timestep
    '''for i in range(Nt):
        print(f"Timestep {i}:")
        for j in range(Ni):
            idx = i * Ni + j
            r_pos = pos_history[0, idx, 0]
            z_pos = pos_history[0, idx, 1]
            vr_vel = vel_history[0, idx, 0]
            vz_vel = vel_history[0, idx, 1]
            erfi = Erfi_history[0, idx]
            ezfi = Ezfi_history[0, idx]
            print(f"  Ion {j}: Position = ({r_pos:.6e}, {z_pos:.6e}), "
                  f"Velocity = ({vr_vel:.6e}, {vz_vel:.6e}), "
                  f"Erfi = {erfi:.6e}, Ezfi = {ezfi:.6e}")'''
