    ...
    for grid_size in grid_sizes:
        ...
        for Ni in ion_counts:
            for shots in shot_sizes:
                ...
                threads_per_block = 128  # Increased for better GPU utilization
                ...
                reorder_all = reorder_all_device.copy_to_host().tolist()
                ...
                with open(file_name, "w") as f:
                    ...
                    for i in range(shots):
                        ...
                        ion_collided = ion_collided_all[i]
                        reorder_val = reorder_all[i]

                        #offset_choice_str = ", ".join([f"{x:.4e}" for x in offset_choice])

                        
                        offset_choice_scalar = offset_choice[0]  

                        
                        output = f"{wz:.2f}\t{velocity:.2f}\t{int(ion_collided)}\t{angle_choice:.4f}\t{offset_choice_scalar}\t{reorder_val}\n"
