...
'''#VF CODE
@cuda.jit(device=True)
def mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder, sum_masses, vf_history):
    ...
    for i in range(Nt):
        ...
        for j in range(Ni):
            for k in range(7):
                vf_history[i * Ni * 7 + j * 7 + k] = vf[j, k]
...
@cuda.jit
def mcCollision_kernel(vf_all, rc_all, zc_all, vrc_all, vzc_all, qc, mc, ac, Nt_all, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder_all, sum_masses_all, vf_history_all):
    idx = cuda.grid(1)
    if idx < vf_all.shape[0]:
        ...
        vf_history = vf_history_all[idx]

        # Call the device function
        mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder, sum_masses, vf_history)
        
        # Store the results back to global memory
        reorder_all[idx] = reorder[0]
        sum_masses_all[idx] = sum_masses[0]
        
        # Save the updated vf back to global memory
        for i in range(MAX_IONS):
            for j in range(7):
...
#     # Fixed values for initial conditions
#     velocity = 1000.0  # m/s
#     angle_choice = np.pi / 4  # 45 degrees
#     offset_choice = 1e-9  # 1 nm
#     ion_collided = 1  # middle ion

#     Nt = 10 #10000000  # Increased number of time steps
...
#     # For GPU version:
#     vf_all = np.array([vf], dtype=np.float64)
#     rc_all = np.array([r], dtype=np.float64)
#     zc_all = np.array([z], dtype=np.float64)
#     vrc_all = np.array([vr], dtype=np.float64)
#     vzc_all = np.array([vz], dtype=np.float64)
#     Nt_all = np.array([Nt], dtype=np.int32)
...
#     #vf_history_all = cuda.device_array((shots, Nt * Ni * 7), dtype=np.float64)


#     Erfi_history_all = cuda.device_array((shots, Nt * Ni), dtype=np.float64)
#     Ezfi_history_all = cuda.device_array((shots, Nt * Ni), dtype=np.float64)
...
#         Erfi_history_all, Ezfi_history_all
#     )

#     # Copy the results back to the host
#     Erfi_history = Erfi_history_all.copy_to_host()
#     Ezfi_history = Ezfi_history_all.copy_to_host()

#     # Print Erfi and Ezfi for each timestep
#     for i in range(Nt):
#         print(f"Timestep {i}:")
#         for j in range(Ni):
#             print(f"  Ion {j}: Erfi = {Erfi_history[0, i * Ni + j]:.6e}, Ezfi = {Ezfi_history[0, i * Ni + j]:.6e}")
...
#     '''vf_history = vf_history_all.copy_to_host()
#     #Erfi_history = Erfi_history_all.copy_to_host()

#     # Print Ezfi and Erfi for each timestep
#     for i in range(Nt):
#         print(f"Timestep {i}:")
#         for j in range(Ni):
#             vf_values = vf_history[0, i * Ni * 7 + j * 7 : i * Ni * 7 + (j+1) * 7]
#             print(f"  Ion {j}: {vf_values}")'''

#     reorder = reorder_device.copy_to_host()
#     sum_masses = sum_masses_device.copy_to_host()
#     #vf_final = vf_device.copy_to_host()[0]
...
#         '''for j in range(Ni):
#             print(f"    Ion {j}: ({vf_final[j, 0]:.6e}, {vf_final[j, 1]:.6e})")
#         print(f"  Ion velocities:")
#         for j in range(Ni):
#             print(f"    Ion {j}: ({vf_final[j, 2]:.6e}, {vf_final[j, 3]:.6e})")
#         print(f"  Ion masses:")
#         for j in range(Ni):
#             print(f"    Ion {j}: {vf_final[j, 5]:.2e}")
#         print(f"  Boundary hit counts:")
#         for j in range(Ni):
#             print(f"    Ion {j}: {vf_final[j, 6]:.0f}")'''
...
@cuda.jit(device=True)
def mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder, sum_masses, Erfi_history, Ezfi_history):
    # Initialize local variables
    reorder[0] = 0
    sum_masses[0] = 0.0
    Nrmid = (Nr - 1) / 2.0
...
@cuda.jit
def mcCollision_kernel(vf_all, rc_all, zc_all, vrc_all, vzc_all, qc, mc, ac, Nt_all, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder_all, sum_masses_all, Erfi_history_all, Ezfi_history_all):
    idx = cuda.grid(1)
    if idx < vf_all.shape[0]:
        # Load vf for this simulation
        vf = cuda.local.array((MAX_IONS, 7), dtype=float64)
        ...
        # Call the device function
        Ezfi_history = Ezfi_history_all[idx]
        Erfi_history = Erfi_history_all[idx]
        
        mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder, sum_masses, Erfi_history, Ezfi_history)
