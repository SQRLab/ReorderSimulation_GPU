...
@cuda.jit(device=True)
def mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, collision_data, dtNow_array, reorder):
    """
    ...
    for i in range(Nt):
        ...
        collision = collisionMode_cuda(rii, rid, vc[0, 6], 0.000000001)
...
@cuda.jit
def mcCollision_kernel(vf_all, rc_all, zc_all, vrc_all, vzc_all, qc, mc, ac, Nt_all, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, collision_data_all, dtNow_all, reorder_all):
    """
    ...
    if idx < vf_all.shape[0]:
        ...
        mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision,
                        nullFields, collision_data_all[idx], dtNow_all[idx], reorder)
...
shot_sizes = [100000]  # Array of shot sizes to test
...
def main():
    ...
    computation_times_file = "computation_times_timestep.json"
    ...
    for grid_size in grid_sizes:
        ...
        for Ni in ion_counts:    
            for shots in shot_sizes:
                ...
                buffer_size = 10
                data_buffer = []
                output_file = f"simulation_results_10k_timestep/{Ni}ionSimulation_{grid_size}_{shots}shots.txt"
                os.makedirs("simulation_results_10k_timestep", exist_ok=True)
                ...
                for i in range(shots):
                    ...
                    velocity = 600 #1385.7357357357357
                    angle_choice = 1.186707467332183
                    offset_choice = -2e-9 #-1.4451520440072538e-09
                    ion_collided = 1
                ...
                mcCollision_kernel[blocks, threads_per_block](
                    ...
                    dtLarge, dtCollision, nullFields_device, collision_data_device, dtNow_device, reorder_device
                )
                ...
                with open('collision_data_0.000000001.txt', 'w') as f:
                    for i in range(shots):
                        for t in range(Nt):
                            if collision_data_all[i, t, 4] != 0:
                                f.write(f"Timestep {t} "
                                        ...
                                        f"Timestep: {collision_data_all[i, t, 4]}\n")
                        f.write("\n")
