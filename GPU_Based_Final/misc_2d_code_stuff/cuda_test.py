...
def ion_position_potential(x):
    ...
    return [
        x[m] - sum([1 / (x[m] - x[n] + np.sign(x[m]-x[n]) * epsilon)**2 for n in range(m)]) +
        sum([1 / (x[m] - x[n] + np.sign(x[m]-x[n]) * epsilon)**2 for n in range(m + 1, N)])
        for m in range(N)
    ]

def calcPositions(N):
    estimated_extreme = 0.6 * N ** 0.765  # Increased spread for better separation
    initial_guess = np.linspace(-estimated_extreme, estimated_extreme, N)
    sol, infodict, ier, mesg = fsolve(ion_position_potential, initial_guess, full_output=True)
    if ier != 1:
        print(f"fsolve did not converge for N={N}. Message: {mesg}")
        return np.full(N, 2.0)  # Fallback positions
    return sol
...
def makeVf(Ni, q, m, l, wr, offsetr, offsetz, vbumpr, vbumpz):
    ...
    if np.any(np.isnan(pos)):
        print(f"calcPositions returned NaNs for Ni={Ni}. Setting positions to default.")
        pos = np.linspace(-1e-6, 1e-6, Ni)  # Default positions
...
# [CUDA device functions remain unchanged, but ensure they handle MAX_IONS correctly]
...
def collisionParticlesFields_cuda(vf, vc, Ni, ErDC, EzDC, ErAC, EzAC, dr, dz, dtNow, Nrmid, Nzmid, Nr, Nz, Erfi, Ezfi, erfc_debug, ezfc_debug):
    ...
    # Define maximum expected numbers
    MAX_NC = 1  # Adjust as per your simulation
    MAX_NI = 4  # Adjust as per your simulation
    Nc = vc.shape[0]  # Actual number used in loops

    # Local arrays using maximum sizes
    ...
    for i in range(Nc):
        ...
        for j in range(Ni):
            ...
            dist_j[i, j] = math.sqrt(sqDist[i, j]) + small  # Prevent division by zero
            ...
            sqDist_j_1p5[i, j] = sqDist[i, j] * dist_j[i, j]  # To compute sqDist_j ** 1.5
...
@cuda.jit(device=True)
def mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder, trajectory_data, logging_enabled):
    ...
    for i in range(Nt):
        ...
        if logging_enabled and timestep_idx < trajectory_data.shape[1]:
            # Log ion positions and velocities
            for ion_idx in range(Ni):
                trajectory_data[ion_idx * 4, timestep_idx] = vf[ion_idx, 0]  # r position
                trajectory_data[ion_idx * 4 + 1, timestep_idx] = vf[ion_idx, 1]  # z position
                trajectory_data[ion_idx * 4 + 2, timestep_idx] = vf[ion_idx, 2]  # r velocity
                trajectory_data[ion_idx * 4 + 3, timestep_idx] = vf[ion_idx, 3]  # z velocity
            
            # Log collisional particle data
            base_idx = Ni * 4
            trajectory_data[base_idx, timestep_idx] = vc[0, 0]  # r position
            trajectory_data[base_idx + 1, timestep_idx] = vc[0, 1]  # z position
            trajectory_data[base_idx + 2, timestep_idx] = vc[0, 2]  # r velocity
            trajectory_data[base_idx + 3, timestep_idx] = vc[0, 3]  # z velocity
...
@cuda.jit
def mcCollision_kernel(vf_all, rc_all, zc_all, vrc_all, vzc_all, qc, mc, ac, Nt_all, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision, nullFields, reorder_all, trajectory_data, logging_indices):
    idx = cuda.grid(1)
    if idx < vf_all.shape[0]:
        ...
        logging_enabled = (idx == logging_indices[0])
        
        # Get the appropriate slice of trajectory data for this shot
        trajectory_slice = trajectory_data if logging_enabled else cuda.local.array((1, 1), dtype=float64)

        # Call the device function
        mcCollision_cuda(vf, rc, zc, vrc, vzc, qc, mc, ac, Nt, dtSmall, RF, DC, Nr, Nz, dr, dz, dtLarge, dtCollision,
                        nullFields, reorder, trajectory_slice, logging_enabled)
...
ion_counts = [4]  # Array of ion counts to test
shot_sizes = [1000]  # Array of shot sizes to test

def main():
    computation_times_file = "computation_times_test.json"
    ...
    shot_to_log = 242  # Specify which shot to log (0-based index)
    max_timesteps = 700000  # Maximum number of timesteps to log
    ...
    for grid_size in grid_sizes:
        ...
        for Ni in ion_counts:    
            for shots in shot_sizes:
                ...
                buffer_size = 10
                data_buffer = []
                output_file = f"simulation_results_test/{Ni}ionSimulation_{grid_size}_{shots}shots.txt"
                os.makedirs("simulation_results_test", exist_ok=True)
                ...
                trajectory_data = cuda.device_array(((Ni + 1) * 4, max_timesteps), dtype=np.float64)
                logging_indices = cuda.to_device(np.array([shot_to_log], dtype=np.int32))
                ...
                mcCollision_kernel[blocks, threads_per_block](
                    ...
                    dtLarge, dtCollision, nullFields_device, reorder_device, trajectory_data, logging_indices
                )

                # Get results back from GPU
                reorder = reorder_device.copy_to_host()

                trajectory_host = trajectory_data.copy_to_host()

                # Save trajectory data to HDF5 file
                trajectory_filename = f"simulation_results_test/trajectory_grid{grid_size}_ions{Ni}_shot{shot_to_log}.h5"
                with h5py.File(trajectory_filename, 'w') as f:
                    # Create datasets
                    f.create_dataset('timesteps', data=np.arange(max_timesteps))
                    
                    # Save ion trajectories
                    for ion_idx in range(Ni):
                        ion_group = f.create_group(f'ion_{ion_idx}')
                        ion_group.create_dataset('r_position', data=trajectory_host[ion_idx * 4])
                        ion_group.create_dataset('z_position', data=trajectory_host[ion_idx * 4 + 1])
                        ion_group.create_dataset('r_velocity', data=trajectory_host[ion_idx * 4 + 2])
                        ion_group.create_dataset('z_velocity', data=trajectory_host[ion_idx * 4 + 3])
                    
                    # Save collisional particle trajectory
                    coll_group = f.create_group('collisional_particle')
                    base_idx = Ni * 4
                    coll_group.create_dataset('r_position', data=trajectory_host[base_idx])
                    coll_group.create_dataset('z_position', data=trajectory_host[base_idx + 1])
                    coll_group.create_dataset('r_velocity', data=trajectory_host[base_idx + 2])
                    coll_group.create_dataset('z_velocity', data=trajectory_host[base_idx + 3])
